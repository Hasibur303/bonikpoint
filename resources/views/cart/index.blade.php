        ...
        @if(count($cartItems))
            <div class="grid gap-5 lg:grid-cols-[1fr_360px] lg:gap-8">
                ...
                <aside class="h-fit rounded-lg bg-white p-4 shadow-sm ring-1 ring-gray-100 md:p-6">
                    ...
                    <a href="{{ route(auth()->check() ? 'checkout.create' : 'guest.checkout.create') }}" class="mt-6 block rounded-lg bg-primary px-5 py-3 text-center text-sm font-black text-white shadow-lg shadow-primary/20 hover:bg-ink md:px-6 md:text-base">Proceed to Checkout</a>
                </aside>
            </div>
        @else
